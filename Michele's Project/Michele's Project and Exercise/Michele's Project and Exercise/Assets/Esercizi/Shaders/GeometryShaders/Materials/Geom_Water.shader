...
Shader "Custom/Geometry/Water" {
	...
	SubShader {
		...
		// this pass only takes whatever is on the screen
		GrabPass{ "_GrabTexture" }

		// note that geometry shaders need passes too
		Pass{
			CGPROGRAM

			#pragma vertex vert             
			#pragma fragment frag
			#include "UnityCG.cginc"	// needed for some parts
			...
			// custom sine function with parameters
			float2 sinusoid(float2 x, float2 m, float2 M, float2 period) {
				float2 excursion = M - m;
				float2 coeff = 3.1415 * 2.0 / period;
				return excursion / 2.0 * (1.0 + sin(x * coeff)) + m;
			}
