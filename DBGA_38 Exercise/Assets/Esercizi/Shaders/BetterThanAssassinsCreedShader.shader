Shader "Custom/BetterThanAssassinsCreedShader" {
	Properties{
	...
	SubShader{
		...
		void surf(Input IN, inout SurfaceOutputStandard o) {
			...
			float t = .5*sin(_Time.w/1 -IN.worldPos.y) + .5;
			
			o.Albedo = lerp(_Color, _Color2, tex2D(_MainTex, float2(uv_x,uv_y)).r);
			o.Alpha = lerp(0.2, 1, tex2D(_MainTex, float2(uv_x, uv_y)).r*t);

			if (t < 0.2)
				o.Emission.gb = tex2D(_MainTex, float2(uv_x, uv_y)).r*t*3;
