...
public class MyNetworkLowLevelManager : MonoBehaviour
{
    ...
    const short MSG_TYPE_BROADCAS_CHAT_TO_CLIENTS = 201;
    ...
    void Awake()
    {
        ...
        else
        {
            ...
            client.RegisterHandler(MSG_TYPE_BROADCAS_CHAT_TO_CLIENTS, OnClientChatMessageReceived);
    ...
    void OnServerChatMessageReceived(NetworkMessage msg)
    {
        ...
        // Chat message arrive from a client
        ChatMessage receivedMessage = msg.ReadMessage<ChatMessage>();

        // Filter message...
        receivedMessage.message = receivedMessage.message.Replace("Tales", "*****");
        

        // Broadcast message to all clients
        NetworkServer.SendToAll(MSG_TYPE_BROADCAS_CHAT_TO_CLIENTS, receivedMessage);
    }
    ...
    {
        Debug.Log("CLIENT: complete connection: - " + msg.channelId + " " + msg.conn.address);
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            newChatMessage.playerName = "Dog ";
            newChatMessage.message = "Ma alla fine Tales Of è o non è un jRPG!? ";
